...
RegisterNetEvent('qb-mechanicjob:server:setVehicleRaised', function(netId, state)
    local veh = NetworkGetEntityFromNetworkId(netId)
    if not DoesEntityExist(veh) then return end
    Entity(veh).state:set('IsVehicleRaised', state, true)
end)

RegisterNetEvent('qb-mechanicjob:server:saveJacks', function(netId, j1, j2, j3, j4)
    local veh = NetworkGetEntityFromNetworkId(netId)
    if not DoesEntityExist(veh) then return end
    Entity(veh).state:set('jackStand1', j1, true)
    Entity(veh).state:set('jackStand2', j2, true)
    Entity(veh).state:set('jackStand3', j3, true)
    Entity(veh).state:set('jackStand4', j4, true)
end)

RegisterNetEvent('qb-mechanicjob:server:deleteJackStand', function(netId)
