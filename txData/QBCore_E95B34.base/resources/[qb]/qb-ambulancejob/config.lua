Config = {}
Config.UseTarget = GetConvar('UseTarget', 'false') == 'true' -- ใช้ระบบ qb-target ในการตอบโต้ (ไม่ต้องแก้ที่นี่ ให้ไปแก้ใน server.cfg โดยเพิ่ม setr UseTarget true)
Config.MinimalDoctors = 2                                    -- จำนวนหมอที่ออนไลน์ขั้นต่ำ เพื่อปิดไม่ให้ใช้ระบบเช็คอินอัตโนมัติของโรงพยาบาล
Config.DocCooldown = 1                                       -- ระยะเวลาคูลดาวน์ระหว่างการเรียกหมอ (NPC) หน่วยเป็นนาที
Config.WipeInventoryOnRespawn = true                         -- เปิด/ปิด การลบไอเทมทั้งหมดในตัวผู้เล่นเมื่อกดเกิดใหม่ที่โรงพยาบาล
Config.RespawnAtNearestHospital = true                       -- เปิด/ปิด การให้ไปเกิดใหม่ที่โรงพยาบาลที่ใกล้ที่สุด
Config.Helicopter = 'polmav'                                 -- โมเดลเฮลิคอปเตอร์ที่อนุญาตให้คนทำอาชีพหมอใช้ได้
Config.BillCost = 2000                                       -- ราคาที่ผู้เล่นต้องจ่ายเมื่อใช้ระบบเช็คอินรักษาของโรงพยาบาล
Config.DeathTime = 300                                       -- ระยะเวลา (วินาที) ก่อนที่ผู้เล่นจะเสียเลือดจนตายและต้องไปเกิดใหม่ที่โรงพยาบาล
Config.ReviveInterval = 360                                  -- ระยะเวลาคูลดาวน์ในการชุบชีวิตผู้เล่นที่อยู่ในสถานะใกล้ตาย (laststand)
Config.MinimumRevive = 300                                   -- ระยะเวลาขั้นต่ำที่ต้องรอเพื่อชุบชีวิตผู้เล่นที่อยู่ในสถานะใกล้ตาย (laststand)
Config.PainkillerInterval = 60                               -- ระยะเวลาการออกฤทธิ์ของยาแก้ปวด (ต่อ 1 เม็ด)
Config.HealthDamage = 5                                      -- ค่าความเสียหายเลือดขั้นต่ำ ก่อนที่ระบบจะเริ่มตรวจสอบการบาดเจ็บตามร่างกาย
Config.ArmorDamage = 5                                       -- ค่าความเสียหายเกราะขั้นต่ำ ก่อนที่ระบบจะเริ่มตรวจสอบการบาดเจ็บตามร่างกาย
Config.ForceInjury = 35                                      -- ค่าความเสียหายสูงสุดที่ได้รับ ก่อนที่จะบังคับให้เกิดการบาดเจ็บที่อวัยวะและเอฟเฟกต์ต่างๆ
Config.AlwaysBleedChance = 70                                -- โอกาส (0-100) ที่จะเกิดอาการเลือดไหลเมื่อถูกโจมตีด้วยอาวุธ
Config.MessageTimer = 12                                     -- ระยะเวลาในการแสดงข้อความแจ้งเตือนอาการบาดเจ็บ/เลือดไหล
Config.AIHealTimer = 20                                      -- ระยะเวลาในการรักษา (วินาที) หลังจากกดเช็คอินกับ NPC
Config.BleedTickRate = 30                                    -- ระยะเวลาห่าง (วินาที) ในแต่ละครั้งที่เลือดจะลดลง (Bleed Tick)
Config.BleedMovementTick = 10                                -- จำนวนวินาทีที่จะถูกหักออกจากรอบเลือดลด หากผู้เล่นมีการขยับตัว (เดิน, วิ่งเหยาะๆ, วิ่งเร็ว)
Config.BleedMovementAdvance = 3                              -- ระยะเวลาที่เพิ่มขึ้นในการคำนวณเลือดไหลเมื่อมีการเคลื่อนที่
Config.BleedTickDamage = 8                                   -- ค่าความเสียหายพื้นฐานที่จะคูณกับระดับการเสียเลือดในทุกๆ รอบ (Tick)
Config.FadeOutTimer = 2                                      -- จำนวนรอบที่เลือดลด (Tick) ก่อนที่หน้าจอจะเริ่มวูบดับ (Fadeout)
Config.BlackoutTimer = 10                                    -- จำนวนรอบที่เลือดลด (Tick) ก่อนที่ผู้เล่นจะหมดสติหน้ามืด (Blackout)
Config.AdvanceBleedTimer = 10                                -- จำนวนรอบที่เลือดลด (Tick) ก่อนที่ระดับความรุนแรงของเลือดไหลจะเพิ่มขึ้น
Config.HeadInjuryTimer = 30                                  -- ระยะเวลา (วินาที) ในการสุ่มตรวจเอฟเฟกต์จากการบาดเจ็บที่ศีรษะ
Config.ArmInjuryTimer = 30                                   -- ระยะเวลา (วินาที) ในการสุ่มตรวจเอฟเฟกต์จากการบาดเจ็บที่แขน
Config.LegInjuryTimer = 15                                   -- ระยะเวลา (วินาที) ในการสุ่มตรวจเอฟเฟกต์จากการบาดเจ็บที่ขา
Config.HeadInjuryChance = 25                                 -- โอกาส (เปอร์เซ็นต์) ที่จะเกิดผลข้างเคียงจากการบาดเจ็บที่ศีรษะ
Config.LegInjuryChance = {                                   -- โอกาส (เปอร์เซ็นต์) ที่จะเกิดผลข้างเคียงจากการบาดเจ็บที่ขา แบ่งตามการเคลื่อนที่
    Running = 50,                                            -- ขณะวิ่งเร็ว
    Walking = 15                                             -- ขณะเดิน
}
Config.MajorArmoredBleedChance = 45                          -- โอกาส (เปอร์เซ็นต์) ที่เลือดจะไหลเมื่อได้รับความเสียหายหนักขณะสวมเกราะ
Config.MaxInjuryChanceMulti = 3                              -- ตัวคูณโอกาสบาดเจ็บสูงสุด (นำค่า HealthDamage มาหารความเสียหายที่ได้รับ)
Config.DamageMinorToMajor = 35                               -- ค่าความเสียหายที่ต้องได้รับเพื่อให้ความเสียหายจากอาวุธเบากลายเป็นการบาดเจ็บหนัก (ตั้งเป็น 100 หากต้องการปิด)
Config.AlertShowInfo = 2                                     -- จำนวนจุดที่บาดเจ็บขั้นต่ำ ก่อนที่ระบบจะส่งข้อความแจ้งเตือนให้ผู้เล่นทราบ

Config.Locations = {                                         -- ส่วนแก้ไขจุดโต้ตอบต่างๆ ในโรงพยาบาล หรือสร้างจุดใหม่
    ['checking'] = {
